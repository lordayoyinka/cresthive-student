...
const RegistrationPage = () => {
  ...
  const createNewUser = async (email, password, token, firstName, lastName) => {
    const auth = getAuth();
    const db = getFirestore();
    ...
    // Define a variable to track if a matching token is found
    ...
    querySnapshot.forEach((doc) => {
      ...
      if (Array.isArray(data.tokens)) {
        ...
        // Find the entry where entry.tokens === token
        const matchingEntry = data.tokens.find(
          (entry) => {
          console.log(entry.tokens, token)

          const str = entry.tokens
        ...
        if (
          matchingEntry &&
          matchingEntry.firstName === firstName &&
          matchingEntry.lastName === lastName
        ) {
    ...
    if (matchingTokenFound) {
      ...
      alert("No matching token found.");


      // You can handle the case where no matching token is found, e.g., display an error message.
      return false; // You might want to return some value to indicate failure.
    }
  };
...
const addStudentToClass = async (className, uid, email) => {
  const db = getFirestore();
  const classDocRef1 = doc(db, year, "1st", "classes", className);
  const classDocRef2 = doc(db, year, "2nd", "classes", className);
  const classDocRef3 = doc(db, year, "3rd", "classes", className);

  // docref 1
  try {
    // Check if the class document already exists
    const classDocSnapshot = await getDoc(classDocRef1);

    if (classDocSnapshot.exists()) {
      // If the document exists, update it
      await updateDoc(classDocRef1, {
        students: arrayUnion({ uid, email }),
      });
    } else {
      // If the document doesn't exist, create it with the student
      await setDoc(classDocRef1, {
        students: [{ uid, email }],
      });
    }
  ...
  try {
    // Check if the class document already exists
    const classDocSnapshot = await getDoc(classDocRef2);

    if (classDocSnapshot.exists()) {
      // If the document exists, update it
      await updateDoc(classDocRef2, {
        students: arrayUnion({ uid, email }),
      });
    } else {
      // If the document doesn't exist, create it with the student
      await setDoc(classDocRef2, {
        students: [{ uid, email }],
      });
    }
  ...
  try {
    // Check if the class document already exists
    const classDocSnapshot = await getDoc(classDocRef3);

    if (classDocSnapshot.exists()) {
      // If the document exists, update it
      await updateDoc(classDocRef3, {
        students: arrayUnion({ uid, email }),
      });
    } else {
      // If the document doesn't exist, create it with the student
      await setDoc(classDocRef3, {
        students: [{ uid, email }],
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    localStorage.setItem("studentyear", formData.year);
      ...
      console.log("tried saving, or failed", user, selectedClass)
  ...
  return (
    <div className="h-full overflow-y-auto h-screen  mx-auto p-6 bg-white shadow-lg rounded-lg">
      <div className="absolute top-0 left-0">
        <Loading newstate={loader} />
        </div>

      <h1 className="text-3xl mt-10 font-semibold mb-6 text-center text-indigo-600">
        ...
        <div className="my-12 p-2 border-[1px] rounded-xl border-slate-300">
          ...
          {isCExpanded && (
            <div className="flex flex-col">
              ...
              {["Literary and Debate Club", "Science Club", "Math Club", "Press Club", "Programming Club", "FRSC Club", "Enterpreneurship Club"].map((activity) => (
                <div key={activity} className="mt-2">
                  <input
                    type="checkbox"
                    name={activity}
