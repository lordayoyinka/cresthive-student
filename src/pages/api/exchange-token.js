...
// ?token=<that ID token>&year=...&term=.... SignIn.jsx calls this route
// with the token. We verify it's genuinely signed by Firebase and not
// expired, then mint a short-lived custom token for the same user so the
// browser can establish a real Firebase Auth session in this app too.
//
// Requires these environment variables to be set in THIS Vercel project
// (cresthive-student) — same values as cresthive_admin's, just added here
// too since Vercel env vars don't carry over between separate projects,
// even when they share the same Firebase project underneath:
